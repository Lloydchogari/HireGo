  ...
  return (
    <div className="container" style={{ paddingBottom: 48 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28, flexWrap: 'wrap', gap: 12 }}>
        ...
        <Link to="/dashboard/new" className="btn btn-accent">+ Post a truck</Link>
      </div>
      ...
      {trucks.map((truck) => (
        <div className="listing-row" key={truck.id}>
          ...
          <div className="listing-row-actions">
            <button className="btn btn-outline" onClick={() => handleToggleStatus(truck)}>
              {truck.status === 'active' ? 'Pause' : 'Activate'}
            </button>
            <Link to={`/dashboard/edit/${truck.id}`} className="btn btn-outline">Edit</Link>
            <button className="btn btn-primary" onClick={() => handleDelete(truck.id)}>Delete</button>
          </div>
        </div>
      ))}
    </div>
